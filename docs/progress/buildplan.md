...
- [x] Make `cmd/key_store` menu continuously navigable: action loop returns to root after each command, submenus accept `e` to go back, root accepts `e` to exit, and terminal clears between root/action transitions
- [x] Make `cmd/key_store` menu state filesystem-consistent across actions: refresh KeyStore memory indexes from disk before each root menu decision so `deep-clean` is immediately reflected by `view`
- [x] Add `stats` action to `cmd/key_store` root menu and CLI args; report runtime/system stats plus on-disk storage breakdown for `data/`, `metadata/`, `.cache/`, and all other entries under storage root
